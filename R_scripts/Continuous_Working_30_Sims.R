...
average_results <- average_results %>%
  pivot_longer(cols = c(S_mean, E_mean, I_mean, V_mean,N_mean),
               names_to = "Compartment", values_to = "count")
ggplot(average_results_30_sims_tau_70, aes(x = time, y = count, color = Compartment)) +
  geom_line(linewidth = 1) +
  theme_minimal() +
  labs(title = paste("Average of", n_sim, "Simulations for Vaccination Rate of 70% per year"),
       x = "Time (days)", y = "Average Count")


average_results_30_sims_tau_70 <- average_results


average_results_30_sims_tau_70 <- average_results_30_sims_tau_70 %>%
